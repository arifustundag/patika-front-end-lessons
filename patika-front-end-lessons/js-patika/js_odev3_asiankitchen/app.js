...
window.onload = () => {
  ...
  btn.innerHTML = categories.map(
    c => `<button class="btn btn-outline-dark btn-item" id="${c}">${c}</button>`).join("");


  //callback function to map html section when click button
  let filteredMenu;
  const handleCards = (filteredMenu) => {
    const section = document.querySelector(".section-center")
    section.innerHTML = filteredMenu.map(item => {
      return `
  ...
  //when the page load default list
